...
#+BEGIN_SRC R
  ...
  unique(fresco_seimitsu$prefecture)
  unique(fresco_taichou$prefecture)


  taichou_all <- dplyr::bind_rows(uncount_blclass(nagasaki),
                                  kumamoto,
                                  uncount_blclass(kagoshima))
  ...
  taichou_all %>%
    dplyr::filter((year == 2018 & month >= 9) |
                  (year == 2019)) %>%
    ggplot(aes(bl)) +
    geom_histogram() +
    facet_grid(prefecture ~ month)
#+END_SRC
** 漁獲量
#+BEGIN_SRC R
  library(hash)
...
#+BEGIN_SRC R
  ...
  coltypes_taichou <- list("測定部位コード"     = "i",
                           ...
                           "開始緯度"           = "d",
                           "開始経度"           = "d")
  prefec_code <- read.csv("~/Downloads/prefecture_code_eng.csv")

  usethis::use_data(coltypes_seimitsu, internal = TRUE, overwrite = TRUE)
  usethis::use_data(coltypes_taichou, internal = TRUE, overwrite = TRUE)
#+END_SRC
*** 体長データ整形
