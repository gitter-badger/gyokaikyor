...
#+BEGIN_SRC R
  ...
  sst_b_mean <- data.frame(logsst = tapply(train[, "logsst"], cycle(train[, "logsst"]), mean),
                           logb   = tapply(train[, "logb"], cycle(train[, "logb"]), mean)) %>%
    as.matrix()
  ...
  sst_b2_mean      <- data.frame(logsst = tapply(train2[, "logsst"], cycle(train2[, "logsst"]), mean),
                                 logb   = tapply(train2[, "logb"], cycle(train2[, "logb"]), mean)) %>%
    as.matrix()
  sarimax_forecast <- auto.arima(y = train2[, "logcatch"],
                                 ...
                                 parallel = TRUE,
                                 num.cores = 4)
  sarimax_f_future <- forecast(sarimax_forecast, xreg = sst_b2_mean)
  ...
  dev.off()
  exp(mean(sarimax_f_future$mean[4:9]))
  make_summary(list_catchdata, 2019, "Mar")
