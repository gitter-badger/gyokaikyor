...
#+BEGIN_SRC R
  ...
  plot_catch_prefec(list, 2019, "Mar")
...
#+BEGIN_SRC R
  ...
  train       <- window(seikai_ts,  end = c(2017, 12))
  sst_b       <- train[, c("logsst", "logb")]
  # xvars_train <- window(xvars, end = c(2018, 12))
  # test        <- window(seikai_ts,  start = c(2018, 1), end = c(2018, 12))
...
#+BEGIN_SRC R tangle R/plot.R
  ...
  plot_catch_prefec <- function(list, year, gkk.month) {
    ...
    plot_init <- function(sums) {
      ...
      axis(1, at = c(1:6, 7.5), labels = prefecs)
      axis(2, at = seq(0, ceiling(max * 1.2), 1))
    }
    ...
    plot_prefec <- function(prefec, sums) {
      ...
      rect(lrecent, recent * 0.8, rrecent, recent * 1.2,
           col = hsv(0, 0, 0.8))
      lines(c(lrecent, rrecent), c(recent, recent), lwd = 2)

      lines(c(x_last, x_this), c(last, this), lwd = 1.5)
      points(x_this, this, pch = 16, col = hsv(0, 0.8, 0.8),
             cex = 1.2, xpd = TRUE)
      points(x_last, last, pch = 16, col = hsv(200/360, 0.8, 0.8),
             cex = 1.2, xpd = TRUE)
    }
